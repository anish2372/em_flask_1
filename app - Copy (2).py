...
import json  # Needed to parse MQTT payloads
...
# def on_mqtt_message(client, userdata, msg):
    ...
    # try:
        ...
        # Emit data to client via SocketIO
        # socketio.emit('device_data', {
            # 'device_key': device_key,
            # 'data': payload
        # }, namespace='/device')
        # device_states[device_key] = data  # store the latest response
        # print(data)
...
def on_mqtt_message(client, userdata, msg):
    ...
    try:
        ...
        socketio.emit('device_data', {
            'device_key': device_key,
            'data': data  # Use data here, not payload
        }, namespace='/device')
        device_states[device_key] = data  # store the latest response
        print(data)
    except Exception as e:
...
# def device_page(device_key):
    ...
    # Publish MQTT request for relay 1 data
    # mqtt_payload = json.dumps({
        # "relay": 1, 
        # "request": "status"
    # })
    # mqtt_client.publish(MQTT_TOPIC_REQUEST.format(mac=device_key), mqtt_payload)

    # Get last known response
    # last_state = device_states.get(device_key)
...
def device_page(device_key):
    ...
    # Create backend API endpoint for mode switching
...
def device_set_mode(device_key):
    ...
    c.execute("SELECT 1 FROM devices WHERE device_key = ?", (device_key,))
    if not c.fetchone():
        conn.close()
        return jsonify({'error': 'Device not found'}), 404

    c.execute("UPDATE devices SET mode = ? WHERE device_key = ?", (mode, device_key))
    conn.commit()
    conn.close()

    return jsonify({'message': f'Mode set to {mode} for device {device_key}'})
